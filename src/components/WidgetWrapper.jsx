...
export function WidgetWrapper({ id, children, defaultPos, defaultSize, visible, onSave, style = {} }) {
  ...
  const [pos, setPos] = useState(() => {
    const saved = localStorage.getItem(`nebula_widget_pos_v3_strict_${id}`);
    return saved ? JSON.parse(saved) : defaultPos;
  });
  const [size, setSize] = useState(() => {
    const saved = localStorage.getItem(`nebula_widget_size_v3_strict_${id}`);
  ...
  useEffect(() => {
    localStorage.setItem(`nebula_widget_pos_v3_strict_${id}`, JSON.stringify(pos));
  }, [pos, id]);

  useEffect(() => {
    localStorage.setItem(`nebula_widget_size_v3_strict_${id}`, JSON.stringify(size));
  }, [size, id]);
